    ...
    return (
        <>
            <div className="flex justify-center items-center flex-wrap m-3">
                <button disabled={!(quedaStock())} onClick={handlerAgregarAlCarrito} className={quedaStock() ? "bg-gray-500 hover:bg-green-500 text-white font-bold py-2 px-4 rounded-full" : "bg-gray-500 text-white font-bold py-2 px-4 rounded-full"} > Agregar al Carrito</button>
                <div className="text-lg font-bold ml-2 mr-2">{cantidadLocal}</div>
                <button disabled={!(estadoCarrito())} onClick={handlerEliminarDelCarrito} className={estadoCarrito() ? "bg-gray-500 hover:bg-red-500 text-white font-bold py-2 px-4 rounded-full" : "bg-gray-500 text-white font-bold py-2 px-4 rounded-full"} >Eliminar del Carrito</button>
            </div>
            <div className="mx-auto text-sm text-blue-600 mt-5 font-semibold">(Stock: {cantidadDisponible})</div>
            <div className="flex items-center">
                {(cantidadLocal !== 0) && <Link to="/cart" className="mx-auto"><button className="mx-auto flex items-center"><img src={irAlCarrito} alt="Ir Al Carrito" className="w-20" /><div className="text-sm bg-gray-500 hover:bg-blue-500 text-white font-bold py-2 px-4 rounded-full ">Finalizar Compra</div></button></Link>}
            </div>
        </>
    )
}
